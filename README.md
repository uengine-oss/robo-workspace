...
Analyzer·Catalog·Data Fabric Python 서비스는 모두 각 저장소 루트의 `.venv`와
`main:app`을 사용합니다. Architect 프로필도 pinned submodule의 각 저장소 루트에서
같은 진입점을 실행합니다. 옛 `app.main:app`과 Data Fabric `backend/` 경로는 지원하지
않으며 `setup`도 루트 `.venv`에 의존성을 설치합니다.
